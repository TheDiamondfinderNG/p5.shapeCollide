...
let title = "%c-=- p5.shapeCollide V0.1.2 (ALPHA) -=-",
...
p5.prototype.collideRectRect = function (x, y, w, h, x2, y2, w2, h2) {
  ...
  if (_renderer._rectMode == CORNER)
    ...
    return x2 + w2 / 2 >= x - w / 2 &&    // r1 right edge past r2 left
      x2 - w2 / 2 <= x + w / 2 &&    // r1 left edge past r2 right
      y2 + h2 / 2 >= y - h / 2 &&    // r1 right edge past r2 left
...
p5.prototype.collideRectCircle = function (rx, ry, rw, rh, cx, cy, diameter) {
  ...
  let testX, testY
  if (_renderer._rectMode == CORNER) {
    testX = cx < rx ? rx : this.min(cx, rx + rw) // left and right edges
    testY = cy < ry ? ry : this.min(cy, ry + rh) // top and bottom edges
  }
  else if (_renderer._rectMode == CENTER) {
...
p5.prototype.collideCircleCircle = function (x, y, d, x2, y2, d2) {
  //2d
  return this.dist(x, y, x2, y2) <= (d + d2) / 2
};
...
p5.prototype.collidePointRect = function (pointX, pointY, x, y, xW, yW) {
  ...
  if (_renderer._rectMode == CORNER)
    ...
    return pointX >= x - xW / 2 &&         // right of the left edge AND
...
p5.prototype.collideLineRect = function (x1, y1, x2, y2, rx, ry, rw, rh, calcIntersection) {
  ...
  let left, right, top, bottom, inside, intersection;
  inside = p5.prototype.collidePointRect(x1, y1, rx, ry, rw, rh) || // Start point inside OR
    p5.prototype.collidePointRect(x2, y2, rx, ry, rw, rh) // End point inside

  // Check edges
  if (_renderer._rectMode == CORNER) {
    left = this.collideLineLine(x1, y1, x2, y2, rx, ry, rx, ry + rh, calcIntersection);
    right = this.collideLineLine(x1, y1, x2, y2, rx + rw, ry, rx + rw, ry + rh, calcIntersection);
    top = this.collideLineLine(x1, y1, x2, y2, rx, ry, rx + rw, ry, calcIntersection);
    bottom = this.collideLineLine(x1, y1, x2, y2, rx, ry + rh, rx + rw, ry + rh, calcIntersection);

  }
  else if (_renderer._rectMode == CENTER) {
    left = this.collideLineLine(x1, y1, x2, y2, rx - rw / 2, ry - rh / 2, rx - rw / 2, ry + rh / 2, calcIntersection);
    right = this.collideLineLine(x1, y1, x2, y2, rx + rw / 2, ry - rh / 2, rx + rw / 2, ry + rh / 2, calcIntersection);
    top = this.collideLineLine(x1, y1, x2, y2, rx - rw / 2, ry - rh / 2, rx + rw / 2, ry - rh / 2, calcIntersection);
    bottom = this.collideLineLine(x1, y1, x2, y2, rx - rw / 2, ry + rh / 2, rx + rw / 2, ry + rh / 2, calcIntersection);
